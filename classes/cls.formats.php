...
function getSalt($str = '', $limit = 6, $opt = 'str' )
{
	$res = array();
	$res['salt'] = substr(md5(uniqid(rand(), true)), 0, $limit);
	$res['pass'] = ($str <> '') ? sha1($str . $res['salt']) : '';
	if($opt == 'str'){
		return substr(md5(uniqid($str, true)), 0, $limit);
	} else {
		return $res; 
...
function displayDecimal($amount) { $out = ($amount <> '') ? number_format(floatval($amount,2,'.',',')) : '' ; return $out; }
function displayFloat($amount)   { $out = ($amount <> '') ? number_format(floatval($amount)) : '' ; return $out; }
...
function ytvid_code($str) {
	$v_insert 		= strrpos($str, '/');
	$v_code			= substr($str, $v_insert+1);
	return $v_code;
}

function ytvid_exists($videoID) {
    $theURL = "https://www.youtube.com/oembed?url=https://www.youtube.com/watch?v=$videoID&format=json";
    $headers = get_headers($theURL);    
    return (substr($headers[0], 9, 3) !== "404");
}

function recursive_array_search($needle,$haystack, $out = array()) {
    
    foreach($haystack as $key => $value) {
        $current_key = $key;
        foreach($value as $key_sub => $value_sub) {
            if($needle === $value_sub){
                $out[$current_key] = $current_key;
            }
        }
    }
    return $out;
...
function dateDisplay($str){
	if($str <> ''){
		//$str_val = ( strtotime($str) ) ? strtotime($str) : $str;
		$str_val = $str;
		$str = date('M d, Y', $str_val);
	}
	return $str;
}
...
function display_noContent ($box_title = '', $htag = 'h2', $hcolor = '')
{
	$result = '';
	//$result = '<div class="subcolumns padd20"><div class="note"><h3>Coming soon!</h3><br>Try again later...</div></div>';
	$result = '<div class="subcolumns padd20"><div class="notex"><!--<h3>Sorry! Nothing to display.</h3>--></div></div>';
	
	return $result;
}

function display_PageTitle ($box_title, $htag = 'h2', $hcolor = '', $html = 0)
{
	$result = '';
	if($box_title <> ''){
		if($htag == '') { $htag = 'h2'; }
		if($hcolor <> '') { $hcolor = ' class="'.$hcolor.'" '; }
		$box_title = ($html == 0) ? clean_title($box_title) : $box_title;
		$result = '<div class="section-title"><'.$htag.''.$hcolor.'><span>'.$box_title.'</span></'.$htag.'></div>';
	}
	return $result;
}

function display_linkMenu ($mlink, $mlinkSeo, $url_only = '')
{
	$link = '#';
...
function display_linkArticle ($cid, $cref, $url_only = '')
{
	$result = $cid.'/'.$cref.'/';
...
/**
 * IMPLODE Key and Value from an Array
 * @return STRING
 */	

function implode_key_val($data_input){

	 $data_output = implode(' | ', array_map(
		function ($v, $k) { return sprintf("%s='%s'", $k, $v); },
		$data_input,
		array_keys($data_input)
	 ));
	 return $data_output;
}		
...
function autoThumbnail($im, $as_is = 0)
{
	...
	else
	{
		...
		if($as_is == 0){
			...
			$pic_relpath  = $pic_src.$im;
		}
		else
		{
			$pic_thmb_a   = SITE_PATH.$pic_thmb;
			$pic_thmb 	  = SITE_DOMAIN_LIVE.$pic_thmb;
			$pic_relpath  = SITE_PATH.$im;
			
		}
		
		if (!file_exists($pic_relpath)) {
			$pic_thmb = ''; return $pic_thmb;
		}
		else 
		{
			if (!file_exists($pic_thmb_a)) 
			{	
				$image_details 	= getimagesize($pic_relpath); //displayArray($image_details); exit;
				$quality = 1;
				...
				$orig_image = $creationFunction($pic_relpath);
				list($width, $height, $type, $attr) = getimagesize($pic_relpath);
				if ($width > $thmb_width) 
...
function getContGalleryPic($cont_id, $cont_title = '', $cont_type = '_cont', $img_options = '', $url_only = '') { 
	$imageReturn = '';
	if(is_array(@master::$listGallery['parent'][$cont_type][$cont_id]))
	{
		...
		elseif($pic_type == 'v')
		{
			...
			$pic_small	  = 'http://img.youtube.com/vi/'.$vid_code.'/hqdefault.jpg';
		}	
...
function checkIfStaff($str) {
	...
	if($u_email_domain == SITE_DOMAIN_URI or $u_email_domain == SITE_DOMAIN_URI_MX) { $staffEmail   = 1; }
	return $staffEmail;	
}
...
function smartTruncateNew($text, $length, $break=' ', $start=0, $suffix = '...', $isHTML = true) 
{ /*&hellip;*/
	$string = $text;
...
function clean_title($string, $caps = 0)
{
...
function daterange_bdays($ndays = 3){ 
	$dleo 	= strtotime(date('Y-m-d'));
	$drange = array();
	$l 		= $dleo;
	while ($l <= ($dleo + (86400*$ndays))) :
		$drange[] = date('m-d', $l);
		$l += 86400;
	endwhile;
	//echobr( implode(',', $drange));
	return $drange;
} 
...
function autoTable($arrVal, $grouper = '', $arrOps = ''){
	
	$add_checks		= (!empty($arrOps['add_checks'])) ? $arrOps['add_checks'] : 0;
	$add_redirect	= (!empty($arrOps['redirect'])) ? $arrOps['redirect'] : 'home.php';
...
//		displayArray($arrVal[0]);
		...
		foreach($arrVal as $t => $rw)
		{
			...
			$row_checkbox	= ($add_checks == 1) ? '<input type="checkbox" name="check['.$t.']" ></td>' : '';
			
			$act_rpt_list .= '<tr><td>'.$row_checkbox.'</td><td data-loc="'.$rw_grouper.'" data-loc-recs="'.$act_rpt_row_recs.'">'. $i .'</td>'. implode('', $act_rpt_row) .'</tr>';
	...
	if($add_checks == 1 )
        {
            ...
            $deFilter = "<form method='post' action='adm_posts.php?tk=".time()."'  id='adm_fm_oservices' target='_blank'> <div style='padding:10px;'>
			...
			$deFooter = "<input type=\"hidden\" name=\"formname\" value=\"adm_fm_oservices\" />
				<input type=\"hidden\" name=\"redirect\" value=\"".$add_redirect."\" />
				<input type=\"hidden\" name=\"calldir\" value=\"".$add_redirect."\" />";
        }
	
	
	
	$deTable = $deFilter.'<div class="padd20">
		<table class="table display dataTable table-responsiveX table-hover" id="example">
		 <thead><tr><th></th><th>#</th><th>'.$col_names.'</th></tr></thead>
...
function autoTable_V0($arrVal, $grouper = '', $add_checks = 0){
